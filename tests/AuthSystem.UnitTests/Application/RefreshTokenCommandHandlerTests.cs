...
public class RefreshTokenCommandHandlerTests
{
    ...
    private RefreshTokenCommandHandler CreateHandler() =>
        new(_userRepository.Object, _jwtService.Object, _tokenHasher, _logger.Object);
    ...
    public async Task Handle_WithValidToken_RotatesRefreshTokenAndReturnsNewAccessToken()
    {
        ...
        TestDataFactory.IssueRefreshToken(user, _tokenHasher, existingToken);
        var refreshToken = EntityTestHelper.GetLatestRefreshToken(user);
        var tokenHash = refreshToken.TokenHash;
        var command = TestDataFactory.CreateRefreshTokenCommand(existingToken);
        var accessExpiry = DateTime.UtcNow.AddMinutes(15);

        _userRepository
            .Setup(r => r.GetByRefreshTokenHashAsync(tokenHash, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new RefreshTokenLookup(user, refreshToken));

        _userRepository
            .Setup(r => r.RevokeActiveRefreshTokenByIdAsync(refreshToken.Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(1);
        ...
        user.RefreshTokens.Should().Contain(t => t.TokenHash == _tokenHasher.Hash("rotated-refresh-token"));
    ...
    public async Task Handle_WhenRevokedTokenReused_RevokesAllSessionsAndThrowsUnauthorizedException()
    {
        ...
        var user = TestDataFactory.CreateVerifiedUserWithRole();
        TestDataFactory.IssueRefreshToken(user, _tokenHasher, reusedToken);
        var refreshToken = EntityTestHelper.GetLatestRefreshToken(user);
        user.RevokeRefreshToken(refreshToken.TokenHash);

        var command = TestDataFactory.CreateRefreshTokenCommand(reusedToken);

        _userRepository
            .Setup(r => r.GetByRefreshTokenHashAsync(refreshToken.TokenHash, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new RefreshTokenLookup(user, refreshToken));
        ...
        _userRepository.Verify(
            r => r.RevokeAllRefreshTokensForUserAsync(user.Id, It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task Handle_WhenTokenExpired_RevokesTokenAndThrowsUnauthorizedException()
    {
        ...
        var user = TestDataFactory.CreateVerifiedUserWithRole();
        TestDataFactory.IssueRefreshToken(user, _tokenHasher, expiredToken);
        var refreshToken = EntityTestHelper.GetLatestRefreshToken(user);
        ...
        _userRepository.Verify(
            r => r.RevokeActiveRefreshTokenByIdAsync(refreshToken.Id, It.IsAny<CancellationToken>()),
            Times.Once);
    }

    [Fact]
    public async Task Handle_WhenConcurrentRotationDetected_RevokesAllSessions()
    {
        const string existingToken = "existing-refresh-token";
        var user = TestDataFactory.CreateVerifiedUserWithRole();
        TestDataFactory.IssueRefreshToken(user, _tokenHasher, existingToken);
